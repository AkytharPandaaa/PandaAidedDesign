...
module cool_frame(pi_version = 3) {
  module frame_shape(pi_version, length = 82.5, mounting_screw_od = 3.5) {
    ...
    height = [0, 0, 2, 3.6, 5][pi_version - 1];
    ...
    difference() {
      ...
      translate([80 + .1, 14, 7]) rotate([90, 0, 270]) mirror([1, 0, 0]) translate([-8.32, 0, 0]) linear_extrude(
                .5 + .1
              ) {
                text(str(pi_version), font="LiberationMono");
              }
    ...
    translate([border_offset, border_offset, 35 + 2]) {
      for (x = [0:1]) {
        for (y = [0:len(locations) - 1]) {
          translate([3.75 + 67 * x, locations[y][1], 0]) mirror([x, 0, 0]) rotate([0, 180 + 45, 0]) cube(
                  [
                    1.5,
                    cable_tie_width,
                    12,
  ...
  union() {
    ...
    // support if using seeed m.2 adapter
    if (pi_version == 5) {
      for (i = [0:3]) {
        translate(v=[90, 10 + 10 * i, 0]) difference() {
            cylinder(h=4.1, d=5.5, center=true);
            cylinder(h=4.3, d=2.8, center=true);
          }
      }
    }
  }
...
for (i = [3:5]) {
  translate(v=[0, 120 * (i - 3), 0]) cool_frame(pi_version=i);
}
